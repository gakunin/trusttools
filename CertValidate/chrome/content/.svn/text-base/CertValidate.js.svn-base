...
var CertValidate = {
	...
	// 処理始まる
	init: function() {
		this.toolbar = document.getElementById('toolbarCertValidate');
		this.toolbarLabel1 = document.getElementById('toolbarLabel1');
		this.toolbarImage1 = document.getElementById('toolbarImage1');
		this.progressmeter = document.getElementById('progressCertValidate');
	...
	initToolbar: function() {
		...
		this.toolbarLabel1.value = "CertValidate: initializing...";
	},
	...
	getPreferences: function() {
		...
		this.prefs = Cc["@mozilla.org/preferences-service;1"]
						...
						.getBranch("extensions.CertValidate.")
	...
	initDB: function() {
		...
		try {
			...
			file.append("CertValidate2.sqlite");

			// Must be checked before openDatabase()
			var exists = file.exists();

			// Now, CertValidate.sqlite exists
	...
	validationProcess: function() {
		...
		// アクセスサイトの証明書は証明書リストに存在しない
			...
			this.toolbarLabel1.value = "CertValidate: 証明書検証対象外";
		}
		// アクセスサイトの証明書取得失敗
		else {
			this.toolbarLabel1.value = "CertValidate: Reload the page to validate again.";
		}
	},
	...
	warn: function(result) {
		this.toolbarLabel1.value = "CertValidate: エラー";
		this.progressmeter.style.visibility = "hidden";
		window.openDialog("chrome://CertValidate/content/warning.xul",
...
const srcCertified = "chrome://CertValidate/content/_certified.gif";
const srcWarning = "chrome://CertValidate/content/_warning.gif";


// ==========BEGIN extension event handling==========
gBrowser.addEventListener("DOMContentLoaded", function(e) { loadCurrent(e); }, false);
if (gBrowser.tabContainer) {
	gBrowser.tabContainer.addEventListener("TabSelect", function() { CertValidate.init(); }, false);
}
